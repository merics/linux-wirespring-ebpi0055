...
static struct platform_driver imx6_wdt_driver = {
	.probe		= watchdog_probe,
	.remove		= watchdog_remove,
	.driver		= {
		.name	= "imx6_wdt",
		.of_match_table = of_match_ptr(imx_wdt_dt_ids),
	},
};

static int __init watchdog_init(void) 
{
	return platform_driver_register(&imx6_wdt_driver);
}

static void __exit watchdog_exit(void)
{
	platform_driver_unregister(&imx6_wdt_driver);
}
