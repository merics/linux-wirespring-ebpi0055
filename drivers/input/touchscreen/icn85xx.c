...
static struct icn85xx_set_dev
{
	unsigned long icn85xx_init_pin;
	unsigned long icn85xx_reset_pin;
}*icn85xx_smg;
...
static int icn85xx_tool_read( char *page, char **start, off_t off, int count, int *eof, void *data )
{
    ...
    else if (0 == cmd_head.wr)   //read para
    {
       ...
       ret = icn85xx_i2c_rxdata(0x8000, &read_data[0], cmd_head.data_len);
        if (ret < 0) {
            icn85xx_error("%s read_data i2c_rxdata failed: %d\n", __func__, ret);
        }
       // memcpy(&g_structPanelPara, &page[0], sizeof(g_structPanelPara));
       memcpy(&g_structPanelPara, &read_data[0], sizeof(g_structPanelPara));
        goto read_out;
    ...
    else if(2 == cmd_head.wr)  //get update status
    {
	  ...
	  read_data[0] = icn85xx_get_status();
	  page[0] = read_data[0];
	 // memcpy(&page[0],&read_data[0], sizeof(read_data[0]));
        proc_info("status: %d\n", page[0]);
    }
    ...
    else if(10 == cmd_head.wr)  //read adc data
    {
	   ...
	   icn85xx_prog_read_page(2500*cmd_head.flag,&read_data[0],cmd_head.data_len);
	    memcpy(&page[0],&read_data[0], cmd_head.data_len);
...
struct icn85xx_ts_data *icn85xx_ts = i2c_get_clientdata(this_client);
    ...
    //if(INVALID_GPIO != icn85xx_ts->rst)
    //{
        //gpio_set_value(icn85xx_ts->rst,0);
       gpio_direction_output(icn85xx_ts->rst, 0);
        msleep(50);
     //  printk("[leewin0]  %s  icn85xx_ts=%d\n",__func__,gpio_get_value(icn85xx_ts->rst));
        //gpio_set_value(icn85xx_ts->rst,1);
       gpio_direction_output(icn85xx_ts->rst, 1);
        msleep(70);
     //  printk("[leewin1]  %s  icn85xx_ts=%d\n",__func__,gpio_get_value(icn85xx_ts->rst));
...
static void icn85xx_report_value_A(void)
{
    ...
    for(i=0; i<icn85xx_ts->point_num; i++)
    {
        ...
        {
            icn85xx_ts->point_info[i].u16PosX = icn85xx_ts->screen_max_x- icn85xx_ts->point_info[i].u16PosX - 1;
        }
        if(1 == icn85xx_ts->revert_y_flag)
        {
            icn85xx_ts->point_info[i].u16PosY = icn85xx_ts->screen_max_y- icn85xx_ts->point_info[i].u16PosY - 1;
        }
	...
	if (1 == icn85xx_ts->point_num)
	{
		input_report_abs(icn85xx_ts->input_dev, ABS_X,icn85xx_ts->point_info[i].u16PosX);
		input_report_abs(icn85xx_ts->input_dev, ABS_Y,icn85xx_ts->point_info[i].u16PosY);
  		input_report_abs(icn85xx_ts->input_dev, ABS_PRESSURE, icn85xx_ts->point_info[i].u8Pressure);
	}
...
***********************************************************************************************/
static void icn85xx_ts_suspend()
{
    struct icn85xx_ts_data *icn85xx_ts = i2c_get_clientdata(this_client);
...
static int icn85xx_request_io_port(struct icn85xx_ts_data *icn85xx_ts)
{
	...
	icn85xx_ts->rst = icn85xx_ts_rst_gpio; //maybe need changed
#endif
...
static int icn85xx_request_input_dev(struct icn85xx_ts_data *icn85xx_ts)
{
    ...
    //#if (LINUX_VERSION_CODE > KERNEL_VERSION(3,6,0))
     ...
     #endif
#else
	set_bit(ABS_X, icn85xx_ts->input_dev->absbit);
		set_bit(ABS_Y, icn85xx_ts->input_dev->absbit);
		set_bit(ABS_PRESSURE, icn85xx_ts->input_dev->absbit);
		set_bit(BTN_TOUCH, icn85xx_ts->input_dev->keybit);
		//__set_bit(BTN_MOUSE, icn85xx_ts->input_dev->absbit);
		//__set_bit(BTN_LEFT, icn85xx_ts->input_dev->absbit);
		//__set_bit(BTN_RIGHT, icn85xx_ts->input_dev->absbit);

		//icn85xx_ts->input_dev->evbit[0] = BIT_MASK(EV_KEY);
		icn85xx_ts->input_dev->keybit[BIT_WORD(BTN_MOUSE)] = BIT_MASK(BTN_LEFT) |
		BIT_MASK(BTN_RIGHT) | BIT_MASK(BTN_MIDDLE);
		//icn85xx_ts->input_dev->relbit[0] = BIT_MASK(REL_X) | BIT_MASK(REL_Y) |
		//BIT_MASK(REL_WHEEL);
		//__set_bit(INPUT_PROP_POINTER, icn85xx_ts->input_dev->absbit);
...
*/
  ...
  //__set_bit(KEY_SEARCH,  input_dev->keybit);
	...
	/*ryl*/
	set_bit(EV_ABS, icn85xx_ts->input_dev->absbit);
	set_bit(EV_KEY, icn85xx_ts->input_dev->absbit);


    icn85xx_ts->input_dev->name = CTP_NAME;
    ret = input_register_device(icn85xx_ts->input_dev);
    if (ret) {
        icn85xx_error("Register %s input device failed\n", icn85xx_ts->input_dev->name);
...
/*
static int icn85xx_ts_fb_event_notify(struct notifier_block *self,
...
static struct notifier_block icn85xx_ts_fb_notifier = {
	.notifier_call = icn85xx_ts_fb_event_notify,
};
*/
static int icn85xx_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//struct device *icn85xx_dev = &client->dev;

	icn85xx_smg = kmalloc(sizeof(struct icn85xx_set_dev), GFP_KERNEL);



		// icn85xx_ts_int_gpio = icn85xx_smg->icn85xx_init_pin;// = 1;
	 icn85xx_ts_rst_gpio = icn85xx_smg->icn85xx_reset_pin;// = 0;
...
// this_client->irq = gpio_to_irq(ft5x06_smg->ft5x06_init_pin);
//this_client->irq = gpio_to_irq(icn85xx_smg->icn85xx_init_pin);
...
//    icn85xx_ts->irq_lock = SPIN_LOCK_UNLOCKED;
	...
	//printk("*******************1*********************");
	icn85xx_smg->icn85xx_init_pin = 3;//of_get_gpio(icn85xx_dev->of_node, 0);
		icn85xx_smg->icn85xx_reset_pin = 4;//of_get_gpio(icn85xx_dev->of_node, 1);
	//	printk("##1icn85xx_smg->icn85xx_init_pin = %d\n", icn85xx_smg->icn85xx_init_pin);
	//	printk("###2icn85xx_smg->icn85xx_reset_pin = %d\n", icn85xx_smg->icn85xx_reset_pin);
		gpio_request(icn85xx_smg->icn85xx_init_pin, "icn85xx_init_pin");
		gpio_request(icn85xx_smg->icn85xx_reset_pin, "icn85xx_reset_pin");

		gpio_direction_output(icn85xx_smg->icn85xx_reset_pin, 1);
		gpio_set_value(icn85xx_smg->icn85xx_reset_pin, 0);
		msleep(70);
		 gpio_set_value(icn85xx_smg->icn85xx_reset_pin, 1);
  ...
  //  icn85xx_ts_reset();
	...
	icn85xx_ts->screen_max_x = SCREEN_MAX_X;
	   ...
	   icn85xx_ts->irq = gpio_to_irq(icn85xx_smg->icn85xx_init_pin); //maybe need changed
	   icn85xx_ts->rst = icn85xx_ts_rst_gpio; //maybe need changed
...
static int __init icn85xx_ts_init(void)
{
	...
	if (ret)
	{
		printk("Adding driver failed\n");
	}
	else
	{
		printk("Successfully add driver %s\n", icn85xx_ts_driver.driver.name);
	}
    return ret;
}
